...
class MyTradingStrategy(AbstractTradingStrategy):
    ...
    # ===================== QUOTING =====================
    def _inv_mid_skew_sigma(self, position: float, sigma_R: float) -> float:
        excess = max(0.0, abs(position) - self.INV_SOFT_LIMIT)
        k_per_unit = 0.25
        cap = 2.0 * sigma_R
        shift = min(cap, k_per_unit * excess * sigma_R)
        return -math.copysign(shift, position)
    def _quote_future(
        self,
        *,
        fair: float,
        ...
        mid = fair + self._inv_mid_skew_sigma(position, sigma_R)
        ...
        # symmetric bid/ask around biased mid
        bid = max(self.MIN_BID, mid - spread / 2.0)
        ask = mid + spread / 2.0
